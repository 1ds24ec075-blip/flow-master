...
/**
 * Routes, keyed on the active company.
 *
 * Changing the key unmounts and remounts every page, discarding component
 * state that would otherwise outlive a switch — a table holding rows in
 * useState, a half-filled form, a selected row id belonging to the previous
 * company. The provider separately clears the react-query cache; between the
 * two, nothing from the old company survives the switch. Cheaper and far
 * harder to get wrong than auditing 21 pages for stale state individually.
 */
function AppRoutes() {
  const { activeCompanyId } = useCompany();

  return (
    <Fragment key={activeCompanyId ?? "no-company"}>
      <Routes>
...
/**
 * CompanyProvider sits above the routes rather than inside ProtectedRoute,
 * because the auth gate here is applied per-route (25 times) and there is no
 * single node inside it to hang this from. It is gated all the same: the
 * provider reads the session itself and resolves to "unauthenticated" with an
 * empty membership list and a null company when there is none, so a public
 * route like /auth or /privacy inherits a context that grants nothing.
 */
const App = () => (
