-- Bill Tally payload/status columns
...
-- No policies: token hashes are readable only by the service role.
...
GRANT SELECT ON public.tally_sync_queue TO authenticated;
GRANT ALL ON public.tally_sync_queue TO service_role;
