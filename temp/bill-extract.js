// supabase/functions/bill-extract/index.ts
...
function normalizeBillNumber(billNumber) {
  ...
  return billNumber.toUpperCase().replace(/[^A-Z0-9]/g, "").replace(/^0+/, "").trim();
}
function amountsMatch(amount1, amount2, tolerance = 1) {
  if (amount1 === null || amount2 === null) return false;
  return Math.abs(amount1 - amount2) <= tolerance;
}
function datesMatch(date1, date2, daysTolerance = 5) {
...
function parseOcrDate(rawValue) {
...
function extractTinNumber(text) {
...
function extractItems(lines) {
  ...
  for (const line of lines) {
    const lowerLine = line.toLowerCase();
    if (/(subtotal|grand total|net total|amount due|balance due|total payable|invoice total|bill total|cgst|sgst|igst|gst|tax|invoice no|bill no|receipt no|date|thank you|terms|round off)/i.test(lowerLine)) {
      continue;
    }
    const numbers = [...line.matchAll(/(?:₹|rs\.?|inr)?\s*([0-9][0-9,]*(?:\.[0-9]{1,2})?)/gi)].map((match) => parseAmount(match[1])).filter((value) => value !== null);
    if (numbers.length < 2) {
      continue;
    }
...
function parseBillFromOcrText(ocrText) {
...
async function checkForDuplicateBill(supabase, currentBillId, vendorName, billNumber, billDate, totalAmount, vendorGst) {
  const noMatch = {
    isDuplicate: false,
    matchedBillId: null,
  ...
  const { data: existingBills, error } = await supabase.from("bills").select("id, bill_number, vendor_name, vendor_gst, bill_date, total_amount").neq("id", currentBillId).order("created_at", { ascending: false }).limit(500);
  ...
  for (const bill of existingBills) {
    if (vendorName && billNumber && bill.vendor_name && bill.bill_number) {
      if (bill.vendor_name.toLowerCase() === vendorName.toLowerCase() && bill.bill_number.toLowerCase() === billNumber.toLowerCase()) {
    ...
    if (normalizedVendor && normalizedBillNum) {
      ...
      if (existingNormalizedVendor === normalizedVendor && existingNormalizedBillNum === normalizedBillNum) {
    ...
    if (vendorGst && billNumber && bill.vendor_gst && bill.bill_number) {
      if (bill.vendor_gst === vendorGst && normalizeBillNumber(bill.bill_number) === normalizedBillNum) {
    ...
    if (normalizedVendor && totalAmount && billDate) {
      const existingNormalizedVendor = normalizeName(bill.vendor_name);
      if (existingNormalizedVendor === normalizedVendor && amountsMatch(bill.total_amount, totalAmount) && datesMatch(bill.bill_date, billDate)) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: fileData, error: downloadError } = await supabase.storage.from("bills").download(bill.image_url);
    ...
    const validatedGst = extracted.vendor_gst && gstPattern.test(extracted.vendor_gst) ? extracted.vendor_gst : null;
    ...
    const { data: updatedBill, error: updateError } = await supabase.from("bills").update({
    ...
    if (extracted.items && extracted.items.length > 0) {
      await supabase.from("expense_line_items").delete().eq("bill_id", billId);
      ...
      const { error: lineItemsError } = await supabase.from("expense_line_items").insert(lineItems);
      if (lineItemsError) {
